...
export default function TranscargoAereoMasterBLIndex() {
  ...
  const { data, isLoading, isError } = useQuery({
    queryKey: ['transcargo-aereo-master-bl'],
    queryFn: async () => {
      const res = await apiClient.get(ENDPOINTS.TRANSCARGO_AEREO_MASTER_BL);
  ...
  const deleteMutation = useMutation({
    mutationFn: (id) => apiClient.delete(ENDPOINTS.TRANSCARGO_AEREO_MASTER_BL + '?id=' + id),
    onSuccess: (_, id) => {
      queryClient.setQueryData(['transcargo-aereo-master-bl'], (old) =>
        old?.filter((item) => item.transcargoAereoMasterBLID !== id) ?? []
      );
      queryClient.invalidateQueries({ queryKey: ['transcargo-aereo-master-bl'] });
  ...
  const columns = [
      ...
      renderCell: (params) => (
        <Box>
          <Tooltip title="Editar">
            <IconButton size="small" color="primary" onClick={() => navigate(`/masterbl/transcargoaereo/edit/${params.row.transcargoAereoMasterBLID}`)}>
              <EditIcon fontSize="small" />
            </IconButton>
          </Tooltip>
          <Tooltip title="Eliminar">
            <IconButton size="small" color="error" onClick={() => setDeleteId(params.row.transcargoAereoMasterBLID)}>
              <DeleteIcon fontSize="small" />
            </IconButton>
          </Tooltip>
          <Tooltip title="Imprimir PDF">
            <IconButton size="small" color="info" onClick={() => navigate(`/masterbl/transcargoaereo/${params.row.transcargoAereoMasterBLID}/print`)}>
    ...
    { field: 'transcargoAereoMasterBLID', headerName: 'ID', width: 70 },
  ...
  return (
    <Box>
      <PageTitle
        title="Master BL - Transcargo Aéreo"
        breadcrumbs={[{ label: 'Inicio', href: '/' }, { label: 'Master BL' }, { label: 'Transcargo Aéreo' }]}
        action={
          <Button variant="contained" startIcon={<AddIcon />} onClick={() => navigate('/masterbl/transcargoaereo/create')}>
            Nuevo Master BL
          </Button>
        }
      />
      ...
      <Box sx={dataGridContainerSx}>
        <DataGrid
          rows={filteredRows}
          columns={columns}
          getRowId={(row) => row.transcargoAereoMasterBLID}
          loading={isLoading}
