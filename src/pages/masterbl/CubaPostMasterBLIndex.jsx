...
export default function CubaPostMasterBLIndex() {
  ...
  const { data, isLoading, isError } = useQuery({
    queryKey: ['cubapost-master-bl'],
    queryFn: async () => {
      const res = await apiClient.get(ENDPOINTS.CUBAPOST_MASTER_BL);
  ...
  const deleteMutation = useMutation({
    mutationFn: (id) => apiClient.delete(ENDPOINTS.CUBAPOST_MASTER_BL + '?id=' + id),
    onSuccess: (_, id) => {
      queryClient.setQueryData(['cubapost-master-bl'], (old) =>
        old?.filter((item) => item.cubaPostMasterBLID !== id) ?? []
      );
      queryClient.invalidateQueries({ queryKey: ['cubapost-master-bl'] });
  ...
  const columns = [
    { field: 'cubaPostMasterBLID', headerName: 'ID', width: 70 },
      ...
      renderCell: (params) => (
        <Box>
          <Tooltip title="Editar">
            <IconButton size="small" color="primary" onClick={() => navigate(`/masterbl/cubapost/edit/${params.row.cubaPostMasterBLID}`)}>
              <EditIcon fontSize="small" />
            </IconButton>
          </Tooltip>
          <Tooltip title="Eliminar">
            <IconButton size="small" color="error" onClick={() => setDeleteId(params.row.cubaPostMasterBLID)}>
              <DeleteIcon fontSize="small" />
            </IconButton>
          </Tooltip>
          <Tooltip title="Imprimir PDF">
            <IconButton size="small" color="info" onClick={() => navigate(`/masterbl/cubapost/${params.row.cubaPostMasterBLID}/print`)}>
  ...
  return (
    <Box>
      <PageTitle
        title="Master BL - CubaPost"
        breadcrumbs={[{ label: 'Inicio', href: '/' }, { label: 'Master BL' }, { label: 'CubaPost' }]}
        action={
          <Button variant="contained" startIcon={<AddIcon />} onClick={() => navigate('/masterbl/cubapost/create')}>
            Nuevo Master BL
          </Button>
        }
      />
      ...
      <Box sx={dataGridContainerSx}>
        <DataGrid
          rows={filteredRows}
          columns={columns}
          getRowId={(row) => row.cubaPostMasterBLID}
          loading={isLoading}
