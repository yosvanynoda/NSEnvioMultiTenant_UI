...
export default function PalcoMasterBLIndex() {
  ...
  const { data, isLoading, isError } = useQuery({
    queryKey: ['palco-master-bl'],
    queryFn: async () => {
      const res = await apiClient.get(ENDPOINTS.PALCO_MASTER_BL);
  ...
  const deleteMutation = useMutation({
    mutationFn: (id) => apiClient.delete(ENDPOINTS.PALCO_MASTER_BL + '?id=' + id),
    onSuccess: (_, id) => {
      queryClient.setQueryData(['palco-master-bl'], (old) =>
        old?.filter((item) => item.palcoMasterBLID !== id) ?? []
      );
      queryClient.invalidateQueries({ queryKey: ['palco-master-bl'] });
  ...
  const columns = [
    { field: 'palcoMasterBLID', headerName: 'ID', width: 70 },
      ...
      renderCell: (params) => (
        <Box>
          <Tooltip title="Editar">
            <IconButton size="small" color="primary" onClick={() => navigate(`/masterbl/palco/edit/${params.row.palcoMasterBLID}`)}>
              <EditIcon fontSize="small" />
            </IconButton>
          </Tooltip>
          <Tooltip title="Eliminar">
            <IconButton size="small" color="error" onClick={() => setDeleteId(params.row.palcoMasterBLID)}>
              <DeleteIcon fontSize="small" />
            </IconButton>
          </Tooltip>
          <Tooltip title="Imprimir PDF">
            <IconButton size="small" color="info" onClick={() => navigate(`/masterbl/palco/${params.row.palcoMasterBLID}/print`)}>
  ...
  return (
    <Box>
      <PageTitle
        title="Master BL - Palco"
        breadcrumbs={[{ label: 'Inicio', href: '/' }, { label: 'Master BL' }, { label: 'Palco' }]}
        action={
          <Button variant="contained" startIcon={<AddIcon />} onClick={() => navigate('/masterbl/palco/create')}>
            Nuevo Master BL
          </Button>
        }
      />
      ...
      <Box sx={dataGridContainerSx}>
        <DataGrid
          rows={filteredRows}
          columns={columns}
          getRowId={(row) => row.palcoMasterBLID}
          loading={isLoading}
