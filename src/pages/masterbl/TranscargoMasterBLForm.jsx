...
export default function TranscargoMasterBLForm({ editMode = false }) {
  ...
  const { data: existing, isLoading: loadingExisting } = useQuery({
    queryKey: ['transcargo-master-bl', id],
    queryFn: async () => {
      const res = await apiClient.get(`${ENDPOINTS.TRANSCARGO_MASTER_BL}/${id}`);
  ...
  const handleApplyTemplate = async () => {
    ...
    try {
      ...
      if (template?.fields?.length) {
        ...
        // Use reset (merged with current values) so MUI TextFields re-render
        // and labels float up correctly over the newly set values.
  ...
  const onSubmit = async (data) => {
    ...
    try {
      let savedId;
      if (editMode) {
        await apiClient.put(`${ENDPOINTS.TRANSCARGO_MASTER_BL}?id=${id}`, payload);
        savedId = parseInt(id, 10);
      } else {
        const res = await apiClient.post(ENDPOINTS.TRANSCARGO_MASTER_BL, payload);
      ...
      setTimeout(() => navigate(`/masterbl/transcargo/${savedId}/print`), 800);
    } catch (err) {
  ...
  const fieldSx = { mb: 0 };
  const vals = watch();
  // Returns InputLabelProps that forces the label up when the field has a value,
  // otherwise lets MUI handle it naturally (up on focus, down when empty/blurred).
  ...
  return (
    <Box>
      <PageTitle
        ...
        breadcrumbs={[
          { label: 'Inicio', href: '/' },
          { label: 'Master BL', href: '/masterbl/transcargo' },
      ...
      <Paper sx={{ p: 3, mb: 3 }}>
        ...
        <Box component="form" onSubmit={handleSubmit(onSubmit)} noValidate>
          {/* Row 1: Envio, Fecha, BillOfLading, BookingNumber */}
          ...
          {/* Row 2: EmpresaEnvia, EmpresaRecibe */}
          ...
          {/* Row 5: OceanCarrier, Buque, Viaje */}
          ...
          {/* Row 6: PortLoading, PortUnloading */}
          <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', sm: '1fr 1fr' }, gap: 2, mb: 2 }}>
          ...
          {/* Row 7: PlaceDelivery, PlaceofReceipt, TypeOfMove */}
          <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', sm: '1fr 1fr 1fr' }, gap: 2, mb: 2 }}>
          ...
          {/* Row 8: Contenedor, TipoContenedor, Seals, Refer */}
          <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', sm: '1fr 1fr', md: '1fr 1fr 1fr 1fr' }, gap: 2, mb: 2 }}>
          ...
          {/* Row 9: NumberPk, PesoLb, PesoKg, VolumenCbm, VolumenCft */}
          ...
          {/* Row 11: Texto (multiline full width) */}
          <Box sx={{ mb: 3 }}>
            <TextField
          ...
          <Stack direction="row" spacing={2}>
            ...
            <Button variant="outlined" onClick={() => navigate('/masterbl/transcargo')}>
              Cancelar
            </Button>
          </Stack>
        </Box>
