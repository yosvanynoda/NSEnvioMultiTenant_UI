...
export default function TranscargoMasterBLIndex() {
  ...
  const { data, isLoading, isError } = useQuery({
    queryKey: ['transcargo-master-bl'],
    queryFn: async () => {
      const res = await apiClient.get(ENDPOINTS.TRANSCARGO_MASTER_BL);
  ...
  const deleteMutation = useMutation({
    mutationFn: (id) => apiClient.delete(ENDPOINTS.TRANSCARGO_MASTER_BL + '?id=' + id),
    onSuccess: (_, id) => {
      queryClient.setQueryData(['transcargo-master-bl'], (old) =>
        old?.filter((item) => item.transcargoMasterBLID !== id) ?? []
      );
      queryClient.invalidateQueries({ queryKey: ['transcargo-master-bl'] });
        ...
        <Box>
          <Tooltip title="Editar">
            <IconButton
              size="small"
              color="primary"
              onClick={() => navigate(`/masterbl/transcargo/edit/${params.row.transcargoMasterBLID}`)}
            >
          ...
          <Tooltip title="Eliminar">
            <IconButton
              size="small"
              color="error"
              onClick={() => setDeleteId(params.row.transcargoMasterBLID)}
            >
          ...
          <Tooltip title="Imprimir PDF">
            <IconButton
              size="small"
              color="info"
              onClick={() => navigate(`/masterbl/transcargo/${params.row.transcargoMasterBLID}/print`)}
            >
  ...
  return (
    <Box>
      <PageTitle
        title="Master BL - Transcargo"
        breadcrumbs={[{ label: 'Inicio', href: '/' }, { label: 'Master BL' }, { label: 'Transcargo' }]}
        action={
          <Button
            variant="contained"
            startIcon={<AddIcon />}
            onClick={() => navigate('/masterbl/transcargo/create')}
          >
            Nuevo Master BL
          </Button>
        }
      ...
      <Box sx={dataGridContainerSx}>
        <DataGrid
          rows={filteredRows}
          columns={columns}
          getRowId={(row) => row.transcargoMasterBLID}
          loading={isLoading}
