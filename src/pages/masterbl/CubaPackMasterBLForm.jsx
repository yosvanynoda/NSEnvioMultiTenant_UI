...
export default function CubaPackMasterBLForm({ editMode = false }) {
  ...
  const { data: existing, isLoading: loadingExisting } = useQuery({
    queryKey: ['cubapack-master-bl', id],
    queryFn: async () => {
      const res = await apiClient.get(`${ENDPOINTS.CUBAPACK_MASTER_BL}/${id}`);
  ...
  const onSubmit = async (data) => {
    ...
    try {
      let savedId;
      if (editMode) {
        await apiClient.put(`${ENDPOINTS.CUBAPACK_MASTER_BL}?id=${id}`, payload);
        savedId = parseInt(id, 10);
      } else {
        const res = await apiClient.post(ENDPOINTS.CUBAPACK_MASTER_BL, payload);
      ...
      setTimeout(() => navigate(`/masterbl/cubapack/${savedId}/print`), 800);
    } catch (err) {
  ...
  return (
    <Box>
      <PageTitle
        ...
        breadcrumbs={[
          { label: 'Inicio', href: '/' },
          { label: 'Master BL', href: '/masterbl/cubapack' },
      ...
      <Paper sx={{ p: 3, mb: 3 }}>
        ...
        <Box component="form" onSubmit={handleSubmit(onSubmit)} noValidate>
          ...
          <Stack direction="row" spacing={2}>
            ...
            <Button variant="outlined" onClick={() => navigate('/masterbl/cubapack')}>
              Cancelar
            </Button>
          </Stack>
        </Box>
