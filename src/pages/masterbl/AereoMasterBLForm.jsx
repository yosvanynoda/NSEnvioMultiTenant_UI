...
export default function AereoMasterBLForm({ editMode = false }) {
  ...
  const { data: existing, isLoading: loadingExisting } = useQuery({
    queryKey: ['aereo-master-bl', id],
    queryFn: async () => {
      const res = await apiClient.get(`${ENDPOINTS.AEREO_MASTER_BL}/${id}`);
  ...
  const onSubmit = async (data) => {
    ...
    try {
      let savedId;
      if (editMode) {
        await apiClient.put(`${ENDPOINTS.AEREO_MASTER_BL}?id=${id}`, payload);
        savedId = parseInt(id, 10);
      } else {
        const res = await apiClient.post(ENDPOINTS.AEREO_MASTER_BL, payload);
      ...
      setTimeout(() => navigate(`/masterbl/aereo/${savedId}/print`), 800);
    } catch (err) {
  ...
  return (
    <Box>
      <PageTitle
        ...
        breadcrumbs={[
          { label: 'Inicio', href: '/' },
          { label: 'Master BL', href: '/masterbl/aereo' },
      ...
      <Paper sx={{ p: 3, mb: 3 }}>
        ...
        <Box component="form" onSubmit={handleSubmit(onSubmit)} noValidate>
          ...
          <Stack direction="row" spacing={2}>
            ...
            <Button variant="outlined" onClick={() => navigate('/masterbl/aereo')}>
              Cancelar
            </Button>
          </Stack>
        </Box>
