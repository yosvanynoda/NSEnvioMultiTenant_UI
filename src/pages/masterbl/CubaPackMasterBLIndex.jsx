...
export default function CubaPackMasterBLIndex() {
  ...
  const { data, isLoading, isError } = useQuery({
    queryKey: ['cubapack-master-bl'],
    queryFn: async () => {
      const res = await apiClient.get(ENDPOINTS.CUBAPACK_MASTER_BL);
  ...
  const deleteMutation = useMutation({
    mutationFn: (id) => apiClient.delete(ENDPOINTS.CUBAPACK_MASTER_BL + '?id=' + id),
    onSuccess: (_, id) => {
      queryClient.setQueryData(['cubapack-master-bl'], (old) =>
        old?.filter((item) => item.cubaPackMasterBLID !== id) ?? []
      );
      queryClient.invalidateQueries({ queryKey: ['cubapack-master-bl'] });
  ...
  const columns = [
      ...
      renderCell: (params) => (
        <Box>
          <Tooltip title="Editar">
            <IconButton size="small" color="primary" onClick={() => navigate(`/masterbl/cubapack/edit/${params.row.cubaPackMasterBLID}`)}>
              <EditIcon fontSize="small" />
            </IconButton>
          </Tooltip>
          <Tooltip title="Eliminar">
            <IconButton size="small" color="error" onClick={() => setDeleteId(params.row.cubaPackMasterBLID)}>
              <DeleteIcon fontSize="small" />
            </IconButton>
          </Tooltip>
          <Tooltip title="Imprimir PDF">
            <IconButton size="small" color="info" onClick={() => navigate(`/masterbl/cubapack/${params.row.cubaPackMasterBLID}/print`)}>
    ...
    { field: 'cubaPackMasterBLID', headerName: 'ID', width: 70 },
  ...
  return (
    <Box>
      <PageTitle
        title="Master BL - CubaPack"
        breadcrumbs={[{ label: 'Inicio', href: '/' }, { label: 'Master BL' }, { label: 'CubaPack' }]}
        action={
          <Button variant="contained" startIcon={<AddIcon />} onClick={() => navigate('/masterbl/cubapack/create')}>
            Nuevo Master BL
          </Button>
        }
      />
      ...
      <Box sx={dataGridContainerSx}>
        <DataGrid
          rows={filteredRows}
          columns={columns}
          getRowId={(row) => row.cubaPackMasterBLID}
          loading={isLoading}
