...
import {
  ...
  Dialog, DialogTitle, DialogContent, DialogActions, FormControl, InputLabel, Select, MenuItem, Chip,
} from '@mui/material';
...
export default function ProductoInventoryForm() {
  ...
  const { user } = useAuth();
  const createdAudit = useRef({ createdBy: '', createdDate: null, agenciaID: 0, retailPrice: 0 });
  ...
  const loadPrecios = useCallback(async () => {
    if (!id) return;
    try {
      const res = await apiClient.get(`${ENDPOINTS.PRODUCTO_INVENTORY_PRECIO_ENVIO}/byInventory/${id}`);
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      const payload = {
        ...data,
        retailPrice: createdAudit.current.retailPrice,
      ...
      if (isEdit) {
        ...
        setTimeout(() => navigate('/administration/productoinventory'), 1200);
      } else {
        ...
        queryClient.invalidateQueries(['productoInventory']);
        setSnackbar({ open: true, message: 'Producto creado. Configure los precios de venta.', severity: 'success' });
  ...
  const handleSavePrecio = async () => {
    ...
    try {
      ...
      if (precioDialog.item) {
        ...
        setPrecios((prev) => [
          ...prev,
          { ...payload, productoInventoryPrecioEnvioID: newId, tipoEnvioName: tipoEnvio?.tipoEnvioName ?? '' },
        ]);
      }
          ...
          <Paper variant="outlined" sx={{ p: { xs: 2, sm: 3 } }}>
            <form onSubmit={handleSubmit(onSubmit)} noValidate>
              <Grid container spacing={2}>
                <Grid size={12}>
                  <Controller name="productoInventoryName" control={control} render={({ field }) => (
                    <TextField {...field} label="Nombre *" fullWidth error={Boolean(errors.productoInventoryName)} helperText={errors.productoInventoryName?.message} />
                  )} />
                </Grid>

                <Grid size={{ xs: 12, sm: 6 }}>
                  <Controller name="costPrice" control={control} render={({ field }) => (
                    <TextField {...field} label="Costo" type="number" fullWidth
                      slotProps={{ htmlInput: { min: 0, step: 0.01 } }}
                      InputProps={{ startAdornment: <InputAdornment position="start">$</InputAdornment> }}
                      error={Boolean(errors.costPrice)} helperText={errors.costPrice?.message} />
                  )} />
                </Grid>
              ...
              <Box sx={{ mt: 3, display: 'flex', gap: 2 }}>
                <Button variant="outlined" startIcon={<ArrowBackIcon />} onClick={() => navigate('/administration/productoinventory')} disabled={loading}>
                  Cancelar
                </Button>
        ...
        <Grid size={{ xs: 12, md: 5 }} sx={{ alignSelf: 'flex-start' }}>
          <Paper variant="outlined" sx={{ p: { xs: 2, sm: 3 } }}>
            <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', mb: 1.5 }}>
              <Typography variant="subtitle1" fontWeight={700}>Precios por Envío</Typography>
              {isEdit && (
            ...
            {!isEdit ? (
              <Typography variant="body2" color="text.secondary" sx={{ py: 2, textAlign: 'center' }}>
                Guarde el producto primero para configurar los precios por tipo de envío.
              </Typography>
            ) : precios.length === 0 ? (
      ...
      <Dialog open={precioDialog.open} onClose={() => setPrecioDialog({ open: false, item: null })} maxWidth="xs" fullWidth>
        ...
        <DialogContent sx={{ display: 'flex', flexDirection: 'column', gap: 2, pt: '16px !important' }}>
          ...
          {precioDialog.item && (
            <TextField
              label="Tipo Envío"
              value={precioDialog.item.tipoEnvioName ?? ''}
              disabled
              fullWidth
              size="small"
            />
        ...
        <DialogActions>
          ...
          <Button
            ...
            disabled={
              (!precioDialog.item && !precioForm.tipoEnvioID) ||
