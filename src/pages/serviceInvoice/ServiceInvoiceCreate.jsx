...
function ItemModal({ open, onClose, onSave, editItem }) {
  ...
  useEffect(() => {
    if (open) {
      reset(editItem ? { descripcion: editItem.descripcion || '', cantidad: editItem.cantidad || 1, precioUnitario: editItem.precioUnitario || 0 } : { descripcion: '', cantidad: 1, precioUnitario: 0 });
    }
  }, [open, editItem, reset]);
  ...
  return (
    <Dialog open={open} onClose={onClose} maxWidth="sm" fullWidth>
      <DialogTitle sx={{ bgcolor: 'primary.main', color: 'white' }}>{editItem ? 'Editar Línea' : 'Agregar Línea'}</DialogTitle>
      <form onSubmit={handleSubmit(onSubmit)}>
        <DialogContent sx={{ pt: 3 }}>
          <Grid container spacing={2}>
            <Grid size={12}>
              <Controller name="descripcion" control={control} render={({ field }) => (
                <TextField {...field} label="Descripción del Servicio *" fullWidth multiline rows={2} error={Boolean(errors.descripcion)} helperText={errors.descripcion?.message} />
              )} />
            </Grid>
            <Grid size={6}>
 ...
 <TextField {...field} label="Precio *" type="number" fullWidth InputProps={{ startAdornment: <InputAdornment position="start">$</InputAdornment> }} slotProps={{ htmlInput: { min: 0, step: 0.01 }, input: { startAdornment: <InputAdornment position="start">$</InputAdornment> } }} error={Boolean(errors.precioUnitario)} helperText={errors.precioUnitario?.message} />
              )} />
            </Grid>
          </Grid>
        </DialogContent>
...
export default function ServiceInvoiceCreate() {
  ...
  useEffect(() => {
    if (isEdit) {
      apiClient.get(`${ENDPOINTS.GENERAL_INVOICE}/${id}`)
        .then(res => {
          const d = res.data?.data || res.data;
          reset({
            fecha: d.invoiceDate ? dayjs(d.invoiceDate).format('YYYY-MM-DD') : dayjs().format('YYYY-MM-DD'),
            numero: d.invoiceNumber || '',
            nota: d.invoiceDescription || '',
          ...
          setItems((d.items || []).map(it => ({
            ...it,
            descripcion:    it.descripcion    ?? it.serviceDescription ?? '',
            cantidad:       it.cantidad       ?? it.quantity           ?? 1,
            precioUnitario: it.precioUnitario ?? it.retailPrice        ?? 0,
          })));
        })
  ...
  const paymentStatus = totalPaid <= 0 ? 1 : totalPaid >= subTotal - 0.01 ? 3 : 2;
  ...
  const onSubmit = async (data) => {
    if (items.length === 0) {
      setSnackbar({ open: true, message: 'Agregue al menos un servicio', severity: 'warning' });
      return;
    }
    setSaving(true);
    try {
      const payload = {
        invoiceDate: data.fecha,
        invoiceDescription: data.nota,
        paymentStatus,
        ...
        zeller:        Number(data.zeller)        || 0,
        items: items.map(it => ({
          generalInvoiceItemID: it.generalInvoiceItemID || 0,
          serviceDescription: it.descripcion,
          quantity: Number(it.cantidad),
          retailPrice: Number(it.precioUnitario),
        })),
      };
      if (isEdit) {
        await apiClient.put(`${ENDPOINTS.GENERAL_INVOICE}/${id}`, payload);
        queryClient.invalidateQueries({ queryKey: ['serviceInvoice'] });
        setSnackbar({ open: true, message: 'Factura actualizada', severity: 'success' });
        setTimeout(() => navigate(`/serviceinvoice/${id}/print`), 1200);
      } else {
        const res = await apiClient.post(ENDPOINTS.GENERAL_INVOICE, payload);
        const newId = typeof res.data === 'number' ? res.data : res.data?.data ?? res.data;
        queryClient.invalidateQueries({ queryKey: ['serviceInvoice'] });
        setSnackbar({ open: true, message: 'Factura creada', severity: 'success' });
        setTimeout(() => navigate(`/serviceinvoice/${newId}/print`), 1200);
      }
    } catch (err) {
  ...
  return (
    <Box>
      <PageTitle
        title={isEdit ? 'Editar Factura' : 'Nueva Factura de Servicio'}
        breadcrumbs={[{ label: 'Inicio', href: '/' }, { label: 'Facturas', href: '/serviceinvoice' }, { label: isEdit ? 'Editar' : 'Nueva' }]}
        action={
          <Button variant="outlined" startIcon={<ArrowBackIcon />} onClick={() => navigate('/serviceinvoice')}>
            Regresar
          </Button>
        }
      />
      ...
      <form onSubmit={handleSubmit(onSubmit)} noValidate>
        ...
        <Paper variant="outlined" sx={{ p: 2, mb: 2 }}>
          <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 2 }}>
            <Typography variant="subtitle1" fontWeight={700} color="primary">Servicios{items.length > 0 && <Chip label={items.length} size="small" color="primary" sx={{ ml: 1 }} />}</Typography>
            <Button variant="contained" startIcon={<AddIcon />} size="small" onClick={() => { setEditItem(null); setEditItemIndex(null); setItemModalOpen(true); }}>Agregar</Button>
          </Box>
          <TableContainer>
            <Table size="small">
              <TableHead>
                <TableRow sx={{ '& th': { bgcolor: 'primary.main', color: 'white', fontWeight: 700 } }}>
                  <TableCell>#</TableCell><TableCell>Descripción</TableCell><TableCell align="right">Cant.</TableCell><TableCell align="right">Precio</TableCell><TableCell align="right">Subtotal</TableCell><TableCell align="center">Acciones</TableCell>
                </TableRow>
              </TableHead>
              <TableBody>
                {items.length === 0 ? (
                  <TableRow><TableCell colSpan={6} align="center" sx={{ py: 3, color: 'text.secondary' }}>No hay servicios. Agregue uno.</TableCell></TableRow>
                ) : (
                  items.map((item, i) => (
        ...
        <Box sx={{ display: 'flex', gap: 2, justifyContent: 'flex-end', pb: 4 }}>
          <Button variant="outlined" startIcon={<ArrowBackIcon />} onClick={() => navigate('/serviceinvoice')} disabled={saving}>Cancelar</Button>
