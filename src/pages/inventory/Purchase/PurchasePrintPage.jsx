...
export default function PurchasePrintPage() {
  ...
  useEffect(() => {
    ...
    fetchPdfBlobUrl(ENDPOINTS.PURCHASE_PDF(id))
      .then((url) => {
  ...
  return (
    <Box sx={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
      <Box sx={{ display: 'flex', alignItems: 'center', gap: 2, p: 2, borderBottom: '1px solid', borderColor: 'divider', bgcolor: 'background.paper' }}>
        <Box sx={{ flex: 1, fontWeight: 700, fontSize: '1.1rem' }}>Compra</Box>
        <Button variant="outlined" size="small" startIcon={<ArrowBackIcon />} onClick={() => navigate('/inventory/purchase')}>
          Regresar
        </Button>
      </Box>
      <PdfViewer
