...
## Pending imported destinatarios visibility fix (2026-08-03)

Full detail in the `project_import_contact_pending_destinatarios` memory. A Remitente imported with 2+ candidate Destinatarios could only ever activate one — the "DESTINATARIOS IMPORTADOS" list in `DestinatarioSection.jsx` was gated on `stagingPhone && !value`, so it vanished the instant any destinatario got picked/created, even though `ImportContact/GetDestinatarios` (already filtered to `IsProcessed=0`) would still return the others. No backend change needed — the endpoint was already correct.

- New `remitentePhone` prop (`remitente?.remitenteTelefono`, passed from `HblCreate.jsx`) drives the pending-imports fetch instead of the one-time `stagingPhone` staging-selection event, so it works for any remitente on any HBL, not just right after an import search.
- Render gate dropped the `!value` condition — the list (now labeled "DESTINATARIOS IMPORTADOS PENDIENTES" with a count `Chip`) stays visible once a destinatario is set, for as long as pending rows remain.
- Activating one now filters just that item out of local state instead of clearing the whole list to `[]`, so a second/third pending destinatario stays activatable.

## Public tracking page: rate-limit debounce + mobile redesign + deliverydate (2026-08-03/04)

User reported intermittent "Error al buscar. Verifique su conexión." on `/rastreo` from a phone. Root cause: the search box fired one request per keystroke with no debounce, against a deliberately tight anonymous rate limit (`hblTracking` policy in `Program.cs`: 10 req/5min/IP, `QueueLimit = 0`) — typing a phone number alone could burn 7-8 requests, worse on mobile where carrier-grade NAT often shares one public IP across many phones. Fixed in `HblTrackingCard.jsx`: 450ms debounce before the query fires (`debouncedQuery` state separate from the raw `search`/`query`), and the error banner now distinguishes a 429 (`error?.status === 429` — the apiClient response interceptor already exposes `status` on the thrown error) from a real connection failure instead of always showing the same generic message. Rate limit itself left unchanged (intentional anti-scraping control).

Same file also got, per explicit user request:
- Mobile cards collapse to header-only by default (Carrier chip, número, estado chip, chevron) — tap to expand full details (Envío/Remitente/Destinatario/Productos/Hace). Desktop table untouched.
- Shipments delivered more than 30 days ago hidden from the default mobile list, with a "Ver todos" link (count of hidden items) to reveal them — still using the same collapsed-card rendering, nothing force-expands.
- This surfaced a real gap: there was no dedicated "delivered" timestamp anywhere — only `fecha` (created/shipped date). Added a proper `deliverydate` column (`NULL` until `hblstatus` becomes 4/Entregado, set exactly then) to all 6 carrier `*hbls` tables via `sp_<carrier>hbl_updatestatus`, which is the single choke point for that transition (`DeliveryWebhookController.cs`, third-party courier delivery-confirmation webhook). **Found while doing this**: `CubaPack`/`CubaPost`/`TranscargoAereo` were missing that function entirely, even though the webhook controller's hardcoded `Carriers` array already referenced all 6 — meaning delivery confirmations silently failed at runtime for exactly those 3 carriers. Created the missing 3 to match the Aereo/Palco/Transcargo pattern. `sp_hbl_trackpublic` now returns `deliverydate` too (required a `DROP FUNCTION` before `CREATE OR REPLACE` — Postgres treats adding a column to a `RETURNS TABLE` signature as a return-type change, which `OR REPLACE` alone can't do).
